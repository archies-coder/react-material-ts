...
export const serverUrl = process.env.REACT_APP_SERVER_URL || 'http://localhost:8000' // http://52.66.55.89:18446/'
...
export async function getInOfficeVisitorData(
  page: number = 0,
  count: number = 10,
  visitor: String = "",
  purpose: String = "",
...
export async function getInOfficeInviteData(
  page: number = 0,
  count: number = 10
) {
...
export async function getCheckInPointsData(
  page: number = 0,
  count: number = 10
) {
  ...
  return {
    //pageLinks: '',
    pageCount: data.totalCount,
    checkInPoints: data.data.map((item: any, i: any) => ({
      ...item,
      checkinpoint_id: i,
    })),
  };
...
export async function signUp(
  ...
  return await apis.post(
    "/product/accountDetail/register",
    JSON.stringify({
      username: username,
      password: password,
      name: name,
...
export async function getVisitorConfigData() {
  ...
  return {
    ...
    //@ts-ignore
    visitorConfigs: Object.keys(data).map((i: string) => ({
      key: i,
      // value: data[i],
    })), //data.data
  };
...
export async function getContractorData(
  ...
  const url = `/product/reception/checkin/user/data?page=${page}&count=${count}&visitor=${contractor}&purpose=${purpose}&site=${site}`;
...
export async function getInOfficeContractorData(
  page: number = 0,
  count: number = 10,
  contractor: String = "",
  purpose: String = "",
